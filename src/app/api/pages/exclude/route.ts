...
export async function POST(request: Request) {
  ...
  if (page.status === "excluded") {
    return NextResponse.json({ error: "Page is already excluded" }, { status: 400 });
  }

  if (page.status === "redirected") {
    return NextResponse.json({ error: "Cannot exclude a redirected page" }, { status: 400 });
  }
