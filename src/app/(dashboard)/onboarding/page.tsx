"use client";

import { useSearchParams } from "next/navigation";
import { Shield, Unplug, Lock } from "lucide-react";
import { OnboardingProgress } from "@/components/onboarding/OnboardingProgress";

const ERROR_MESSAGES: Record<string, string> = {
  access_denied: "You denied access. We need read-only access to your Search Console data.",
  missing_params: "Something went wrong with the authorization. Please try again.",
  auth_mismatch: "Session mismatch. Please try again.",
  no_refresh_token: "Google didn't provide a refresh token. Please try again and make sure to click 'Allow'.",
  token_exchange: "Failed to connect to Google. Please try again.",
};

export default function OnboardingConnectPage() {
  const searchParams = useSearchParams();
  const error = searchParams.get("error");

  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh]">
      <OnboardingProgress currentStep={1} />
      <div className="max-w-md w-full text-center">
        <h1 className="text-2xl font-semibold text-[#111827] mb-2">
          Connect Google Search Console
        </h1>
        <p className="text-[#4B5563] mb-8">
          We need read-only access to your search data to monitor your content. You can disconnect anytime.
        </p>

        {error && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6 text-sm text-red-700">
            {ERROR_MESSAGES[error] ?? "An error occurred. Please try again."}
          </div>
        )}

        <a
          href="/api/auth/google-gsc"
          className="inline-flex items-center justify-center gap-3 w-full h-14 rounded-lg bg-white border-2 border-[#E5E7EB] text-[#111827] text-[15px] font-semibold hover:border-[#3B82F6] hover:shadow-md transition-all"
        >
          <svg viewBox="0 0 24 24" width="22" height="22">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"/>
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
          </svg>
          Connect Google Search Console
        </a>

        <div className="flex items-center justify-center gap-6 mt-8 text-xs text-[#9CA3AF]">
          <span className="flex items-center gap-1.5">
            <Shield size={14} strokeWidth={1.5} />
            Read-only access
          </span>
          <span className="flex items-center gap-1.5">
            <Unplug size={14} strokeWidth={1.5} />
            Disconnect anytime
          </span>
          <span className="flex items-center gap-1.5">
            <Lock size={14} strokeWidth={1.5} />
            Data stays private
          </span>
        </div>
      </div>
    </div>
  );
}
