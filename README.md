...
>
> **Vibecoded with Claude Code** — 313 commits, 312 of them co-authored by a Claude model (every commit but the first). Every commit was a pair session, so the git history cannot separate a human-typed line from a model-generated one, and I won't pretend otherwise.
>
> **What was mine:** the specification and architecture came before the code — the initial commit contains 11 documents, including the full SQL schema in [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md). [`CLAUDE.md`](CLAUDE.md) is the operating manual the assistant worked under, published here in Portuguese exactly as it was used: it locks the stack ("never suggest alternatives unless asked"), forbids `any`, requires Zod on every boundary and RLS on every table, and bans the LLM from anything deterministic math can compute. Over 300 commits later the project still matches its day-one architecture doc. Mine too are the decisions that came out of things breaking in production: the four-provider fallback chain ordered by blast radius, the brief generator skipping the strongest model because it timed out, Firecrawl replacing Cheerio with Cheerio kept as fallback, and the four-step ladder for malformed JSON.
>
...
- **Per account, lifetime:** 10 AI analyses, as a single shared pool — a GSC diagnosis and a standalone URL analysis draw from the same allowance, because they run the same pipeline at the same cost. This started as two separate pools (10 GSC, 3 standalone) on the theory that a standalone URL, requiring no proof of site ownership, is the cheaper abuse vector. The theory was fine and the UI was not: the sidebar counted one pool while the settings page named the other, so the number a free user saw was never the number that actually stopped them. Usage is counted from table rows, never a counter column, because counters drift and rows don't.
...
- **One gate, not four.** Every AI run passes `checkUsage(client, { …, action })` in [`src/lib/limits.ts`](src/lib/limits.ts), which runs the rate limit, the shared free cap, the canceled-subscription check and the monthly paid quota in that order and returns either a refusal or nothing. The order is load-bearing: the cheap in-memory check runs first and every one of them runs before a cent is spent. A new endpoint gets all four by calling one function.
...
This project was vibecoded with Claude Code, and that is the method, not a footnote: **313 commits, 312 of them co-authored by Claude models** — every commit except the first (the trailers are in the git log).
...
- **The stack was locked against drift.** The repo's `CLAUDE.md` instructs the assistant to never suggest stack alternatives unless asked, so over 300 commits later the project still matches its day-one architecture doc.
...
- **Monetization is designed and integrated but disabled in this public version.** Stripe checkout, billing portal, and webhooks are in the codebase and deliberately switched off; the pricing page shows the planned tiers with purchasing disabled. Free usage is capped rather than gated behind upgrades — 10 AI analyses per account, lifetime, shared across Search Console diagnoses and standalone URL analyses, because each run costs real API money — and an `AI_DISABLED` env var can kill all AI calls at the single choke point every call funnels through.
...
- **37 known vulnerabilities in the dependency tree** (`npm audit`: 1 critical, 16 high, 18 moderate, 2 low), all with `fixAvailable: true`. The critical one is `protobufjs`, pulled in transitively through `posthog-js`. They are unfixed for a specific reason: with no automated tests, `npm audit fix` across 37 packages would be a change nobody could verify — the cure would risk more than the disease. *What I'd do first: the CI gate listed below, with `npm audit` in it, so the number is visible on every push instead of discovered by whoever runs the command; then upgrade in small batches once there are tests to catch what breaks.*
