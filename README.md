...
**Vibecoded with Claude Code**, solo, over 283 commits — 282 of them co-authored by Claude. That's the method, documented in full below: what the AI wrote, what I decided, and which decisions I can actually defend.

**It is not a finished product.** No automated tests, no CI, no paying users, monetization deliberately switched off, and the quality of the AI diagnoses has never been systematically evaluated. It ran in production on my own sites; everything it claims to have measured, it measured there.
...
Postgres (Supabase) with Row Level Security on every table; 27 SQL migrations, 13 tables. Deployed on Vercel with cron jobs for ingestion, scoring, alerting, and digests.
