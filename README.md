...
- **36 known vulnerabilities in the dependency tree** (`npm audit`: 1 critical, 15 high, 18 moderate, 2 low), all with `fixAvailable: true`. The critical one is `protobufjs`, pulled in transitively through `posthog-js`. They are unfixed for a specific reason: with no automated tests, `npm audit fix` across 36 packages would be a change nobody could verify — the cure would risk more than the disease. *What I'd do first: the CI gate listed below, with `npm audit` in it, so the number is visible on every push instead of discovered by whoever runs the command; then upgrade in small batches once there are tests to catch what breaks.*
...
- **Plan-limit checks are only half-centralized.** `src/lib/limits.ts` now owns how free usage is *counted* — `countFreeAnalysesUsed` and `getUsage`, which every route and every screen reads, so the displayed number and the enforced number cannot drift apart again. What still lives inline in each API route is the rest of the gate: the rate limit, the canceled-subscription check, the monthly paid quota. *What I'd do first: fold those into a single `checkUsage(user, action)` that returns a refusal or a go-ahead, so a new endpoint gets all four checks by calling one function instead of remembering four.*
